...
tracker_urls = []
if (b'announce' in torrent and False):
    tracker_url = torrent[b'announce']
    trackers_url.append(tracker_url)
else:
    urls = torrent[b'announce-list']
    for url in urls:
        tracker_urls.append(url[0])
...
for url in tracker_urls:
    ...
    if (len(peers_list)==0):
        print("No seeders found. Trying next")
    else:
